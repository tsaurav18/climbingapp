...
(function ($) {
	window.__current_rate = 0;
    $.fn.mdbRate = function (listener) {
      var $stars;
      ...
      var $container = $(this);
      $container.attr("data-current-rate" , 0);
  
      var titles = ['Very bad', 'Poor', 'OK', 'Good', 'Excellent'];
  
      for (var i = 0; i < 5; i++) {
        $container.append(
    			"<i class='py-2 px-1 rate-popover' " +
//    			"	data-index='${i}' " +
    			"data-index='" + i + "'" +  
    			"data-html='true' " +
    			"data-toggle='popover' " +
          ...
          if ($container.hasClass('rating-faces')) {
            ...
            switch (index) {
              case '0':
                ...
                $container.attr("data-current-rate" , 1);
                break;
              case '1':
                $($stars.get(i)).addClass('twoStars');
                $container.attr("data-current-rate" , 2);
                break;
              case '2':
                $($stars.get(i)).addClass('threeStars');
                $container.attr("data-current-rate" , 3);
                break;
              case '3':
                $($stars.get(i)).addClass('fourStars');
                $container.attr("data-current-rate" , 4);
                break;
              case '4':
                $($stars.get(i)).addClass('fiveStars');
                $container.attr("data-current-rate" , 5);
                break;
            }
          } else {
      ...
      $stars.on('click', function (evt, target) {
        $stars.popover('hide');
        
        if(typeof listener === "function"){
        	listener(Number($container.attr("data-current-rate")));
        }
      });
  ...
  $('#rateMe2').mdbRate(function(rate){
	  window.__current_rate = rate;
  });